...
/**
 * Selo Visual de Proveniência Científica — SAREL (§19 e Regra 3)
 *
 * ● medido      — verde, sólido
 * ◊ modelado    — azul, losango
 * □ tabelado    — amarelo, quadrado
 * ○ indisponivel— cinza, círculo aberto (sempre com a causa)
 */
export function SeloProveniencia<T>({
  ...
  let valorFormatado = "indisponível";
  if (proveniencia && proveniencia.estado !== "indisponivel") {
    if (typeof proveniencia.valor === "number") {
  ...
  return (
    ...
    >
      ...
      {expandido && (
        <div className="mt-2.5 border-t border-slate-200 pt-2 text-[11px] not-italic leading-relaxed text-slate-700">
          <p className="font-medium text-slate-800">{descricao}</p>
          {proveniencia && proveniencia.estado === "indisponivel" && (
            <p className="mt-1 text-slate-600">
              <span className="font-semibold">Causa:</span> {proveniencia.causa}
            </p>
          )}
          {proveniencia && proveniencia.estado === "medido" && (
            <div className="mt-1 space-y-0.5 text-slate-500">
              <p>Fonte: {proveniencia.fonte}</p>
              <p>Adquirido em: {proveniencia.adquiridoEm}</p>
              <p>Consultado em: {proveniencia.consultadoEm}</p>
            </div>
          )}
          {proveniencia && proveniencia.estado === "modelado" && (
            <div className="mt-1 space-y-0.5 text-slate-500">
              <p>Modelo: {proveniencia.modelo}</p>
              <p>Insumos: {proveniencia.insumos.join(", ")}</p>
            </div>
          )}
        </div>
      )}
