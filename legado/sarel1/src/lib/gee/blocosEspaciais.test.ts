...
describe("Blocos Espaciais e Semivariograma Empírico (Fase 4 — SAREL)", () => {
  ...
  describe("Atribuição de Bloco Espacial Regular", () => {
    it("atribui identificadores de bloco espacial consistentes (ex: BLOCO_R01_C02)", () => {
      const b1 = atribuirBlocoEspacial(-25.5, -53.5, 20.0);
      const b2 = atribuirBlocoEspacial(-25.5005, -53.5005, 20.0); // Ponto muito próximo
      const b3 = atribuirBlocoEspacial(-24.0, -51.0, 20.0);       // Ponto distante
  ...
  describe("Semivariograma Empírico e Determinação do Bloco (Corrige I4)", () => {
    it("aplica fallback documentado de 20 km para amostra pequena (n < 15)", () => {
      const poucosPontos = [
    ...
    it("calcula semivariograma com pares de dados suficientes", () => {
      // Cria 40 pontos com gradiente espacial de declividade
      const pontos = Array.from({ length: 40 }, (_, i) => ({
        latitude: -25.0 - (i * 0.05),
        longitude: -53.0 - (i * 0.05),
        valor: 5.0 + Math.sin(i * 0.3) * 3.0,
      }));

      const res = calcularSemivariogramaEmpirico(pontos, 5.0, 50.0);
      expect(res.bins.length).toBeGreaterThan(0);
      expect(res.arestaBlocoAdotadaKm).toBeGreaterThanOrEqual(10);
