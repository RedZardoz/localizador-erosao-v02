...
/**
 * ============================================================================
 * Enumeração do Domínio de Erodibilidade — Embrapa GeoInfo
 * Verificação Regra 8 executada em 08/09/2026 sobre geonode:brasil_erodibilidade_solo
 * ============================================================================
 *
 * Classes pedológicas válidas no SiBCS / levantamento nacional da Embrapa:
 * - "Muito Baixa" -> ordinal 1
 * - "Baixa"       -> ordinal 2
 * - "Média"       -> ordinal 3
 * - "Alta"        -> ordinal 4
 * - "Muito Alta"  -> ordinal 5
 *
 * Categorias não-pedológicas (excluídas do frame de modelagem):
 * - "Area urbana" (codnum 9 verificado) -> não-pedológica
 * - "Corpo d'agua" / "Agua"            -> não-pedológica
 * - "Afloramento rochoso"              -> não-pedológica
 */

export interface ResultadoMapeamentoErodibilidade {
  classeOriginal: string;
  ehPedologica: boolean;
  ordinal: number | null; // 1 a 5 para pedológicas, null para não-pedológicas ou desconhecidas
  nivelK: 1 | 2 | null;  // 1 = Baixa/Média (<= 3), 2 = Alta/Muito Alta (>= 4) para estratificação 3x3x2
  motivo?: string;
}

export function mapearErodibilidadeParaOrdinal(classeBruta: string | null | undefined): ResultadoMapeamentoErodibilidade {
  if (!classeBruta || typeof classeBruta !== "string") {
    return {
      ...
      ordinal: null,
      nivelK: null,
      motivo: "Classe de erodibilidade ausente ou não informada.",
    };
  }

  const normalizada = classeBruta.trim().toLowerCase();

  // Categorias não-pedológicas conhecidas
  if (normalizada.includes("urbana") || normalizada.includes("urban")) {
    return {
      classeOriginal: classeBruta,
      ehPedologica: false,
      ordinal: null,
      nivelK: null,
      motivo: "Categoria não-pedológica ('Área urbana'). Ponto não elegível para amostragem agrícola.",
    };
  }
  if (normalizada.includes("agua") || normalizada.includes("água") || normalizada.includes("corpo d'agua")) {
    return {
      classeOriginal: classeBruta,
      ehPedologica: false,
      ordinal: null,
      nivelK: null,
      motivo: "Categoria não-pedológica ('Corpo d'água').",
    };
  }
  if (normalizada.includes("afloramento") || normalizada.includes("rocha")) {
    return {
      classeOriginal: classeBruta,
      ehPedologica: false,
      ordinal: null,
      nivelK: null,
      motivo: "Categoria não-pedológica ('Afloramento rochoso').",
    };
  }

  // Classes pedológicas
  if (normalizada.includes("muito baixa") || normalizada === "muito baixa") {
    return { classeOriginal: classeBruta, ehPedologica: true, ordinal: 1, nivelK: 1 };
  }
  if (normalizada.includes("baixa") || normalizada === "baixa") {
    return { classeOriginal: classeBruta, ehPedologica: true, ordinal: 2, nivelK: 1 };
  }
  if (normalizada.includes("media") || normalizada.includes("média") || normalizada === "media" || normalizada === "média") {
    return { classeOriginal: classeBruta, ehPedologica: true, ordinal: 3, nivelK: 1 };
  }
  if (normalizada.includes("muito alta") || normalizada === "muito alta") {
    return { classeOriginal: classeBruta, ehPedologica: true, ordinal: 5, nivelK: 2 };
  }
  if (normalizada.includes("alta") || normalizada === "alta") {
    return { classeOriginal: classeBruta, ehPedologica: true, ordinal: 4, nivelK: 2 };
  }

  // Classe desconhecida / fora do mapa
  return {
    classeOriginal: classeBruta,
    ehPedologica: false,
    ordinal: null,
    nivelK: null,
    motivo: `Classe de erodibilidade '${classeBruta}' não reconhecida no domínio documentado da Embrapa.`,
  };
}
