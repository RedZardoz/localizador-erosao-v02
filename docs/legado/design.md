...
4. [Fluxos de Trabalho e Diagrama de Estados do Usuário](#4-fluxos-de-trabalho-e-diagrama-de-estados-do-usuário)
5. [Guia de Estilo, Cores e Tipografia](#5-guia-de-estilo-cores-e-tipografia)
...
1. **Cabeçalho Técnico do Ponto:**
   - **Identificador Amostral:** Código padronizado único (ex: `PR-CAND-001`).
   - **Score de Prioridade Top-N:** Pontuação composta de risco de 0 a 100 (ex: `84 pts`).
   - **Badge de Severidade:** *Crítica* (Vermelho), *Alta* (Laranja) ou *Moderada* (Amarelo).
   - **Toponímia:** Município e macrobacia hidrográfica correspondente (ex: `Tibagi (PR)`).
...
## 4. Fluxos de Trabalho e Diagrama de Estados do Usuário

O diagrama a seguir sintetiza a jornada de interação do pesquisador na plataforma, desde a configuração inicial de credenciais até a publicação dos resultados científicos:

```mermaid
flowchart TD
    Start([Abertura da Aplicação]) --> LoadData[Carregamento do Dashboard 2D/3D]
    
    LoadData --> CheckAuth{Credencial GEE Ativa?}
    CheckAuth -- Não / Inserir --> Settings[3.2. SettingsModal: Inserção de Chaves JSON / Tokens]
    Settings --> TestConn[Teste de Conexão GEE & APIs]
    TestConn --> CheckAuth
    CheckAuth -- Sim --> SelectAOI[3.9. RegionRequestModal: Seleção de Região / Município IBGE]
    
    SelectAOI --> SampleAction{Origem dos Focos?}
    SampleAction -- Triagem Automática --> GEEModal[3.8. CandidateSelectionModal: Amostragem GEE]
    GEEModal --> ProcessGEE[Máscara 10m + Estratificação + Thinning]
    ProcessGEE --> ViewPoints[Exibição dos Focos na Sidebar & Mapa 3D]
    
    SampleAction -- Coleção Anterior --> LoadSaved[3.4. SavedDatasetsModal: Carregar Projeto Salvo]
    LoadSaved --> ViewPoints
    
    ViewPoints --> Interaction{Ação do Pesquisador}
    
    Interaction -- Inspecionar Ponto --> Popup[3.5. PointPopup: Raio-X RUSLE & Dados SICAR/INCRA]
    Popup -- Visualizar Divisa --> ShowCAR[Projetar Perímetro da Fazenda no Mapa]
    Popup -- Emitir Laudo --> AuditModal[3.6. AuditDossierModal: Dossiê 6 Etapas & PDF]
    AuditModal --> ExportPDF[Download do Laudo Técnico Oficial em PDF]
    
    Interaction -- Delimitar Área --> DrawTool[3.3. DrawingToolbar: Desenho Vetorial de Talhão]
    DrawTool --> SavePoly[Modal Salvar Talhão: Categorias & Métricas]
    SavePoly --> ViewPoints
    
    Interaction -- Exportar Dados --> ExportHub[3.7. ExportModal: Central de Exportação]
    ExportHub --> ExportFiles[Download GeoJSON / KML 3D / Shapefile / CSV / XLSX / XGBoost]
    
    Interaction -- Salvar Sessão --> SaveProj[3.4. Salvar Coleção Atual / Exportar .JSON]
    SaveProj --> End([Sessão Preservada com Sucesso])
```

---

## 5. Guia de Estilo, Cores e Tipografia
